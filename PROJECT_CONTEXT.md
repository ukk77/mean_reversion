...
> `mean_reversion` · 21 files (python×21) · 27 classes · 25 functions · ~3,540 source lines
...
graph TD
    ...
    generator --> volume
    engine --> loss_guard[trading_core]
    momentum --> base
    sizer --> config
    sizer --> generator
    tracker --> config
...
> Stationarity tests for mean-reversion validation (ADF test).
- `def adf_pvalue(series: Series, lookback: int) → Optional`  — Compute the ADF test p-value on the last N observations.
